...
```
#### 1.d
#### Creates the vector (4,6,3) and assigns to name tmp
```{r}
tmp <- c(4,6,3)
tmp
```
...
```
#### 1.f 
#### Repeats the vector until the desired length (31) occurs
```{r}
x5 <- rep_len(tmp, 31)  
x5
```
...
#### Creates a vector (x) from 3 to 6 in increments of .1
#### seq() for numeric arguments returns a vector
```{r}
x <- seq(3, 6, by = .1)
```
#### Creates a vector (f) for each value in the vector x inputted into the function
```{r}
f <- exp(x)*cos(x)
f
```
...
#### 4.a
#### Creates the vector (1,2,3,...,99,100) 
```{r}
i <- c(10:100)
```
...
```
#### 4.b
#### Creates the vector (1,2,3,...,24,25)
```{r}
i <- c(1:25)
```
...
#### 6.a
#### Will need these numbers for our new vectors
```{r}
i <- c(1:249)
```
...
i <- c(1:248)
```
#### Needed to shrink the i's by one to account for i+2 ending at 250
```{r}
...
#### 7.d
#### Takes the square root of the absolute value of each difference of the components of xVec and the mean of xVec. xVec by itself just outputs x1, x2, .., xns
```{r}
