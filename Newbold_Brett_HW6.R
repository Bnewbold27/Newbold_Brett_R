...
# duplicate the examples as best as I could because displaying in RStudio is dependent on the dimensions of the
# user's plot window
...
p1 <- ggplot(diamonds, aes(price, ..density..)) + # Using density on the y-axis, price for x-axis
  geom_histogram(aes(colour = factor(color)), binwidth = 50) + # Making the plot a histogram with specific binwidth
  # while stacking the variable color from diamonds
  theme(
...
p2 <- ggplot(diamonds, aes(carat, ..density..)) + # Using density on the y-axis, carat for the x-axis
  geom_histogram(aes(colour = factor(color)), binwidth = .0287) + # Making the plot a histogram with specific binwidth 
  # while stacking the variable color from diamonds
  theme(
    legend.position="none", # Making the legend disappear
    axis.title = element_blank(), # Stripping the titles on the axes
